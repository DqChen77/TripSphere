 ...
 * <ul>
 *   <li>uid - User ID
 *   <li>roles - Comma-separated list of roles
...
public class GrpcAuthContext {
    ...
    private static final Metadata.Key<String> UID_KEY =
            Metadata.Key.of("uid", Metadata.ASCII_STRING_MARSHALLER);
    ...
    public static GrpcAuthContext fromMetadata(Metadata metadata) {
        ...
        String uid = metadata.get(UID_KEY);
        String rolesStr = metadata.get(ROLES_KEY);
        ...
        return GrpcAuthContext.builder().userId(uid).roles(roles).token(token).build();
    }
